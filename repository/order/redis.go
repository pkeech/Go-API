...
import (
	...
	"github.com/pkeech/Go-API/model"
...
func (r *RedisRepo) Insert(ctx context.Context, order model.Order) error {
	...
	if err != nil {
		return fmt.Errorf("failed to encode order. \n Msg: %w", err)
	}
	...
	if err := res.Err(); err != nil {
		txn.Discard()
		return fmt.Errorf("failed to set order. \n Msg: %w", err)
	}

	if err := txn.SAdd(ctx, "orders", key).Err(); err != nil {
		txn.Discard()
		return fmt.Errorf("failed to add to orders set. \n Msg: %w", err)
	}
...
func (r *RedisRepo) FindByID(ctx context.Context, id uint64) (model.Order, error) {
	...
	if err != nil {
		return model.Order{}, fmt.Errorf("failed to decode order. \n Msg: %w", err)
	}
...
func (r *RedisRepo) FindAll(ctx context.Context) (FindResult, error) {
	...
	for i, x := range xs {
		...
		if err != nil {
			return FindResult{}, fmt.Errorf("failed to decode order json. \n Msg: %w", err)
		}
